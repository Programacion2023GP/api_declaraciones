...
class ControllerBeneficiosPrivados extends Controller
{
    public function create(Response $response, Request $request)
    {
        ...
        try {
            ...
            foreach ($request->all() as $datos) {
                ...
                DB::table('DECL_BeneficiosPrivados')->insert($datos);
            }



            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Se insertaron los BENEFICIOS PRIVADOS';
            $response->data["alert_text"] = "regimenes encontrados";
            // $response->data["result"] = $InteresId;

            $apartado = new ControllerApartados();

            $apartado->interes($firstRequestItem['Id_Intereses'], 6);
            // $response->data["result"] = $DatosCurriculares;
        } catch (\Exception $ex) {
            $erros = new ControllerErrors();
            error_log('Ocurrió un error: ' . $ex->getMessage());
            $erros->handleException('DECL_BeneficiosPrivados', $ex);
    ...
    public function index(Response $response, int $id)
    {
        ...
        try {
            $data = DB::table('DECL_BeneficiosPrivados') // Selecciona la tabla DECL_Datosgenerales
...

    {
        ...
        try {
            ...
            foreach ($request->all() as $datos) {
                ...
                unset($datos['Id_User']);
                unset($datos['Id_SituacionPatrimonial']);
                unset($datos['Id_BeneficiosPrivados']);
            ...
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria | BENEFICIOS PRIVADOS actualizadas correctamente.';

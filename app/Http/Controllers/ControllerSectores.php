...
class ControllerSectores extends Controller
{
    ...
    public function index(Response $response)
    {
        ...
        try {
            $regimen = DB::table('Sector')->select('valor as text', 'clave as id')->where('active', 1)->orderBy('id', 'desc')
                ->get();
    ...
    public function create(Response $response, Request $request)
    {
        ...
        try {
            $maximo_id = DB::table('Sector')->max('clave');

            $regimen_matrimonial = DB::table('Sector')->insertGetId([
                'clave' => $maximo_id + 1,
                'valor' => $request->valor,
            ]);
            ...
            // return "fff";
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'peticion satisfactoria |  SECTOR QUE PERTENECE guardado correctamente.';
            $response->data["alert_text"] = "regimenes encontrados";
            $response->data["result"] = $regimen_matrimonial;
        } catch (\Exception $ex) {
            // $response->data = ObjResponse::CatchResponse($ex->getMessage());
            $erros = new ControllerErrors();
            $erros->handleException('catalogo_Sector', $ex);
    ...
    public function update(Response $response, Request $request, $id)
    {
        ...
        try {
            ...
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria |  SECTOR QUE PERTENECE actualizado correctamente.';
            $response->data["alert_text"] = "Regímenes encontrados";
            $response->data["result"] = $id; // Puedes devolver el ID del   SECTOR QUE PERTENECE actualizado si lo necesitas
        } catch (\Exception $ex) {
            $erros = new ControllerErrors();
            $erros->handleException('catalogo_Sector', $ex);
            $response->data = ObjResponse::CatchResponse($ex);
    ...
    public function delete(Response $response, Request $request, $id)
    {
        ...
        try {
            ...
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria |  SECTOR QUE PERTENECE eliminado correctamente.';
            $response->data["alert_text"] = "Regímenes encontrados";
            $response->data["result"] = $id; // Puedes devolver el ID del   SECTOR QUE PERTENECE actualizado si lo necesitas
        } catch (\Exception $ex) {
            $erros = new ControllerErrors();

...
class ControllerVehiculos extends Controller
{
    public function show(Response $response)
    {
        ...
        try {
            ...
            $response->data["message"] = 'peticion satisfactoria | lista de vehiculos.';
    ...
    public function index(Response $response)
    {
        ...
        try {
            $regimen = DB::table('TipoVehiculo')->select('valor as text', 'clave as id')->where('active', 1)->orderBy('clave', 'desc')
                ->get();
    ...
    public function create(Response $response, Request $request)
    {
        ...
        try {
            $maximo_id = DB::table('TipoVehiculo')->max('clave');

            $regimen_matrimonial = DB::table('TipoVehiculo')->insertGetId([
                'clave' => $maximo_id + 1,
                'valor' => $request->valor,
            ]);
            ...
            // return "fff";
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'peticion satisfactoria | VEHICULOS guardado correctamente.';
            $response->data["alert_text"] = "regimenes encontrados";
            $response->data["result"] = $regimen_matrimonial;
        } catch (\Exception $ex) {
            // $response->data = ObjResponse::CatchResponse($ex->getMessage());
            $erros = new ControllerErrors();
            $erros->handleException('catalogo_vehiculo', $ex);
    ...
    public function update(Response $response, Request $request, $id)
    {
        ...
        try {
            ...
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria | VEHICULOS actualizado correctamente.';
            $response->data["alert_text"] = "Regímenes encontrados";
            $response->data["result"] = $id; // Puedes devolver el ID del  VEHICULOS actualizado si lo necesitas
        } catch (\Exception $ex) {
            $erros = new ControllerErrors();
            $erros->handleException('catalogo_vehiculo', $ex);
            $response->data = ObjResponse::CatchResponse($ex);
    ...
    public function delete(Response $response, Request $request, $id)
    {
        ...
        try {
            ...
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria | VEHICULOS eliminado correctamente.';
            $response->data["alert_text"] = "Regímenes encontrados";
            $response->data["result"] = $id; // Puedes devolver el ID del  VEHICULOS actualizado si lo necesitas
        } catch (\Exception $ex) {
            $erros = new ControllerErrors();

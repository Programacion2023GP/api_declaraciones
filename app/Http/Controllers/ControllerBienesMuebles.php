...
class ControllerBienesMuebles extends Controller
{
    ...
    public function index(Response $response, int $id)
    {
        ...
        try {
            $data = DB::table('DECL_BienesMuebles') // Selecciona la tabla DECL_DatosGenerales
...

    {
        ...
        try {
            ...
            foreach ($request->all() as $datos) {
                ...
                unset($datos['identificador']);
                // Insertar los datos en la tabla 'DECL_BienesMuebles'
                DB::table('DECL_BienesMuebles')->insert($datos);
            }


            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria | BIENES MUEBLES actualizados correctamente.';

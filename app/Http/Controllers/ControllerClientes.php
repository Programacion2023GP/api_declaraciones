...
class ControllerClientes extends Controller
{
    public function create(Response $response, Request $request)
    {
        ...
        try {
            ...
            foreach ($request->all() as $datos) {
                ...
                DB::table('DECL_ClientesPrincipales')->insert($datos);
            }



            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Se insertaron los CLIENTES PRINCIPALES';
            $response->data["alert_text"] = "regimenes encontrados";
            // $response->data["result"] = $InteresId;

            $apartado = new ControllerApartados();

            $apartado->interes($firstRequestItem['Id_Intereses'], 5);
            // $response->data["result"] = $DatosCurriculares;
        } catch (\Exception $ex) {
            $erros = new ControllerErrors();
            $erros->handleException('DECL_ClientesPrincipales', $ex);
    ...
    public function index(Response $response, int $id)
    {
        ...
        try {
            $data = DB::table('DECL_ClientesPrincipales') // Selecciona la tabla DECL_DatosGenerales
...

    {
        ...
        foreach ($request->all() as $datos) {
            ...
            unset($datos['Id_User']);
            unset($datos['Id_SituacionPatrimonial']);
            unset($datos['Id_ClientesPrincipales']);
            ...
            $response->data = ObjResponse::CorrectResponse();
            $response->data["message"] = 'Petición satisfactoria | CLIENTES PRINCIPALES actualizadas correctamente.';
